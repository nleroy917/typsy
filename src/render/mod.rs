//!
//! This module provides the implementation of the `World` trait for the static site generator (SSG). It defines the `ContentRenderer` struct, which serves as the main interface for Typst to interact with the SSG environment. This includes handling file requests, managing the standard library, and providing access to fonts and the current date.
//! 
//! I was heavily inspired by this example repo: https://github.com/tfachmann/typst-as-library
//! 
...
impl typst::World for ContentRenderer {
    /// Standard library.
